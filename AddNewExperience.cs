...
{
    public partial class AddNewExperience : Form
    {
        private int ID;
        private Log l; 
        ...
        private void AddNewExperience_Load_1(object sender, EventArgs e)
        {
            try
            {
        ...
        public void Exp_bind(string Name)
        {
            if ( dt.Rows.Count == 0)
            {
                dt = w.Select("");
        ...
        private void Save_button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                w.Insert(Name_textBox.Text);
                l.Insert_Log("Insert " + Name_textBox.Text, "Work", Properties.Settings.Default.username, DateTime.Now); 
                Name_textBox.Clear();
                ID = -1; 
        ...
        private void ExperienceName_textBox_TextChanged(object sender, EventArgs e)
        {
            try
            { 
                Exp_bind(Name_textBox.Text);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
            ...
            try
            {
                ...
                if (e.ColumnIndex == dataGridView.Columns["DeleteRow"].Index)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        w.Delete(ID);
                        l.Insert_Log("Delete Work: " + dataGridView.Rows[e.RowIndex].Cells["Name"].Value,
                            "Work", Settings.Default.username, DateTime.Now);
