...
{
    public partial class Taxi_Visit3s_Form : Form
    {
        public Taxi_Visit3s_Form(MainForm mainForm)
        {
            InitializeComponent();
            this.mainForm = mainForm;
            this.V_ID = -1;
        }
        public Taxi_Visit3s_Form(int V_ID, MainForm mainForm)
        {
            InitializeComponent();
            this.mainForm = mainForm;
            this.V_ID = V_ID;
        }
        public Taxi_Visit3s_Form(MainForm mainForm, int MicroProject_ID)
        {
            InitializeComponent();
            this.mainForm = mainForm;
        ...
        private const string _KIND = "T3s";
        DataRow SelectedDataRow;
        int MicroProject_ID, V_ID, Person_ID, User_ID;
        MySqlComponents MySS;
        NewTheme NewTheme;
        Log l; Select s; Visit v;
        DataTable Visit_Users_dt;
        TasksOfProjects TasksOfProjects;
        bool Update_Mood;
        MainForm mainForm;

        private void Taxi_Visit3s_Form_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                mainForm.TabName_label.Text = "Third Visit of Car";
        ...
        private void MicroProject_ID_textBox1_Leave(object sender, EventArgs e)
        {
            ...
            {
                DataTable dt = v.SearchByID(MicroProject_ID_textBox1.Text, false);
                clear_Visit_boxes(0);
        ...
        private void Person_Name_textBox1_Leave(object sender, EventArgs e)
        {
            ...
            {
                DataTable dt = v.SearchByName(Person_Name_textBox1.Text, false);
                clear_Visit_boxes(0);
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            if (dt.Rows[i]["ID"] != DBNull.Value)
                            {
                                if (dt.Rows[i]["Kind"].Equals(_KIND))
                                {
                                    ...
                                    //Ans4_textBox.Text = (string)dt.Rows[i]["Ans4"];
                                    //Ans5_textBox.Text = (string)dt.Rows[i]["Ans5"];
                                    //Ans6_textBox.Text = (string)dt.Rows[i]["Ans6"];
...
ELSE N'Success' End as 'التقييم'" +
        ...
        private void Visits_DataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                if (SelectedDataRow != null)
                {
                    ...
                    Form Evaluation_Form1 = new Evaluation_Form(Evaluation_ID, Beneficiary_ID, "T3s", mainForm);
        ...
        private void clear_Visit_boxes(int j)
        {
            ...
            Ans1_textBox.Text = Ans2_textBox.Text = Ans3_textBox.Text = Ans6_textBox.Text =
                Ans4_textBox.Text = Ans5_textBox.Text = Indicatores_textBox.Text = "";
            //Ans6_textBox.Text = 
