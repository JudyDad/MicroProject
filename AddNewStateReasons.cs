...
{
    public partial class AddNewStateReasons : Form
    {
        private int ID;
        private Log l;
        private StateReason s; 
        private System.Data.DataTable dt;

        public AddNewStateReasons()
        {
            InitializeComponent();
        }

        private void AddNewStateReasons_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void Save_button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                s.Insert(Name_textBox.Text);
                l.Insert_Log("Insert " + Name_textBox.Text, "State Reason", Properties.Settings.Default.username, DateTime.Now);
                Name_textBox.Clear();
                ID = -1; 
            ...
            try
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    if (e.ColumnIndex == 1) // name
                    {
                        ...
                        s.Update(ID, Name);
                        l.Insert_Log("Update State Reason:" + Name,
                            "State Reason", Settings.Default.username, DateTime.Now);
                    }
            ...
            try
            {
                ...
                if (e.ColumnIndex == dataGridView.Columns["DeleteRow"].Index)
                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        s.Delete(ID);
                        l.Insert_Log("Delete State Reason: " + dataGridView.Rows[e.RowIndex].Cells["Name"].Value,
                            "State Reason", Settings.Default.username, DateTime.Now);
