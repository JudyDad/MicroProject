...
namespace MyWorkApplication.Classes.CheckBoxComboBox
{
    ...
    ///     ----------------
    ///     ALSO IMPORTANT: In Data Binding when setting the DataSource. The ValueMember must be a bool type property, because
    ///     it will
    ///     be binded to the Checked property of the displayed CheckBox. Also see the DisplayMemberSingleItem for more
    ///     information.
    ///     ----------------
    ///     Extends the CodeProject PopupComboBox "Simple pop-up control"
    ...
    public class CheckBoxComboBoxListControl : ScrollableControl
    {
        ...
        protected override void WndProc(ref Message m)
        {
            if ((Parent.Parent as Popup).ProcessResizing(ref m)) return;
            base.WndProc(ref m);
        }
            ...
            for (var Index0 = 0; Index0 <= _CheckBoxComboBox.Items.Count - 1; Index0++)
            {
                ...
                else
                {
                    var StartIndex = HasHiddenItem
                        ...
                        : 0;
                    for (var Index1 = StartIndex; Index1 <= Items.Count - 1; Index1++)
